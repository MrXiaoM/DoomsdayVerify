...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (Bukkit.getPluginManager().getPlugin("CoreProtect") != null) {
			this.getServer().getPluginManager().registerEvents(new CoreProtectListener(this), this);
		}
		...
		if (sender.isOp()) {
			if (args.length >= 1) {
				...
				if(args[0].equalsIgnoreCase("time") && args.length == 3) {
					...
					sender.sendMessage(msg("set-time").replace("%player%", player).replace("%time%", String.valueOf(time)));
					return true;
				}
			}
		}
